<div class='news-add-window pb-4'>
  <h4 class='text-center title'>Новая новость</h4>
  <div class=''>
    <form class='form news-item-form'>
      <div class='row my-4'>
        <div class='col-6'>
          <label>
              Заголовок
              <input class='form-control' name='title'>
          </label>
          <label>
              Подзаголовок
              <input class='form-control' name='subtitle'>
          </label>
          <label>
              Категории
              <select class='w-100' name='categories[]' multiple>
                @foreach ($categories as $category)
                  <option value='{{ $category->id }}' data-background='{{ $category->background_color }}' data-color='{{ $category->font_color }}'>{{ $category->name }}</option>
                @endforeach
              </select>
          </label>
          <label>
              Картинка
              <input type='file' accept='image/*' class='form-control' name='image'>
          </label>
          <label>
              Текст
              <textarea class='form-control' name='text' rows='20'></textarea>
          </label>
        </div>
        <div class='col-6 preview'>
          <div class='col-12 px-3 news-item-wrap'>
            <div class='card news-item shadow-sm'>
              <div class='news-item__head no-wrap'>
                <div class='news-item__title-wrap'>
                  <h3 class='news-item__title text-truncate interactable'></h3>
                </div>
                <div class='news-item__subtitle-wrap text-muted interactable'>
                  <strong class='news-item__subtitle text-truncate'></strong>
                </div>
                <div class='news-item__categories text-muted text-truncate'>
                </div>
              </div>
              <div class='news-item__content'>
                  <div class='news-item__image-wrap interactable'>
                      <img src='' class='img-fluid news-item__image'>
                  </div>
                  <div class='w-100 news-item__text text-truncate'>
                  </div>
              </div>
              <div class='news-item__footer'>
                  <div class='d-flex justify-content-center'>
                      <div class='news-item__comments text-muted interactable'>
                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-chat-square-text" viewBox="0 0 16 16">
                              <path d="M14 1a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1h-2.5a2 2 0 0 0-1.6.8L8 14.333 6.1 11.8a2 2 0 0 0-1.6-.8H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h12zM2 0a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2.5a1 1 0 0 1 .8.4l1.9 2.533a1 1 0 0 0 1.6 0l1.9-2.533a1 1 0 0 1 .8-.4H14a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2z"/>
                              <path d="M3 3.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5zM3 6a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 6zm0 2.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5z"/>
                          </svg>
                          <div class='text-center'>
                              999
                          </div>
                      </div>
                  </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </form>
  </div>
  <div class='mt-4'>
    <button class='btn btn-outline-success btn-submit'>Отправить</button>
  </div>
</div>